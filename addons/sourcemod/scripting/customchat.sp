...
{
	g_iPlayerInfo[client].ChatColor = 0;
	g_iPlayerInfo[client].NameColor = 3;
	Format(g_iPlayerInfo[client].Tag, 32, "");
...
public void DB_LoadColors(Database db, DBResultSet results, const char[] error, any data)
{
	...
	if(chat)
		g_iPlayerInfo[client].ChatColor = results.FetchInt(chatcol);
	if(name)
		g_iPlayerInfo[client].NameColor = results.FetchInt(namecol);
	if(tag)
		results.FetchString(tagcol, g_iPlayerInfo[client].Tag, sizeof(ChatData::Tag));
}
...
public void DB_UpdateColors(int client)
{
	...
	if (!GetClientAuthId(client, AuthId_Steam2, steamid, sizeof(steamid)))
		return;
	Format(sQuery, sizeof(sQuery), "INSERT INTO customchat (steamid, chatcolor, namecolor, tag) VALUES ('%s', %d, %d, '%s') ON DUPLICATE KEY UPDATE chatcolor=VALUES(chatcolor), namecolor=VALUES(namecolor), tag=VALUES(tag);", steamid, g_iPlayerInfo[client].ChatColor, g_iPlayerInfo[client].NameColor, g_iPlayerInfo[client].Tag);
...
public int MenuHandler_ChatColor(Menu menu, MenuAction action, int client, int choice)
{
	...
	g_iPlayerInfo[client].ChatColor = choice;
	DB_UpdateColors(client);
}
...
public int MenuHandler_NameColor(Menu menu, MenuAction action, int client, int choice)
{
	...
	g_iPlayerInfo[client].NameColor = choice;
	DB_UpdateColors(client);
}
...
public Action CP_OnChatMessage(int& author, ArrayList recipients, char[] flagstring, char[] name, char[] message, bool& processcolors, bool& removecolors)
{
	...
	if (chat)
	{
		if (g_iPlayerInfo[author].ChatColor != 0)
		{
			Format(message, MAX_MESSAGE_LENGTH, "%s%s", g_sColorCodes[g_iPlayerInfo[author].ChatColor], message);
			changed = true;
		}
	...
	if (n)
	{
		if (g_iPlayerInfo[author].NameColor != 3)
		{
			Format(name, MAX_NAME_LENGTH, " %s%s", g_sColorCodes[g_iPlayerInfo[author].NameColor], name);
			changed = true;
			needspace = true;
		}
	}
